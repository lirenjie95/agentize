...
使用外部 AI 评审（Codex、Cursor Agent CLI 或 Claude Opus），从多 agent 辩论报告中合成一份平衡的共识实现计划。
...
该 skill 使用三级回退策略，以利用可用的最佳高级推理工具：

#### 第一级：Codex CLI（首选）

该 skill 首选 Codex CLI 的高级特性以获得最佳的共识评审效果：
...
#### 第二级：Cursor Agent CLI（中间回退）

当 Codex 不可用但 Cursor Agent CLI 可用且带有 GPT-5.2-Codex-xhigh 时，将其用作中间回退：

**安装**：Cursor Agent CLI（PATH 中的 `agent` 命令）

**检测**：
```bash
command -v agent &> /dev/null       # 检查 CLI 可用性
```

**用法模式**：
```bash
cat input.md | agent -p > output.txt  # 通过 agent -p 标志管道输入
```

**使用的特性**：
- **gpt-5.2-codex-xhigh 模型**：高级推理能力
- **外部研究**：通过 Agent CLI 路由
- **文件 I/O**：标准输入/输出重定向

**优势**：
- Codex 不可用时仍可访问高级推理模型
- 与 Claude 类似的研究能力
- 成本：取决于 Cursor Agent CLI 配置
- 时间：与 Claude 相近（1-3 分钟）

#### 第三级：Claude Code CLI（最终回退）

当 Codex 和 Cursor Agent CLI 都不可用时，回退到使用 Opus 的 Claude Code：
...
5. 加载并处理提示词模板，进行变量替换
6. 应用三级回退策略：
   - **第一级**：检查 Codex 是否可用 → 使用带 xhigh 推理的 Codex
   - **第二级**：检查 Agent CLI 是否可用且带有 gpt-5.2-codex-xhigh → 使用 Agent CLI
   - **第三级**：回退到带只读工具的 Claude Opus
7. 以适当的配置调用外部 AI：
   - **Codex**：gpt-5.2-codex、只读沙箱、web 搜索、xhigh 推理
   - **Agent CLI**：通过 Cursor Agent CLI 使用 gpt-5.2-codex-xhigh
...
- 外部评审者提供**中立、无偏**的视角
- **三级回退级联**：Codex → Agent CLI → Claude Opus
  - 在可用时最大化对高级推理模型的访问
  - 在添加中间选项的同时保留现有行为
  - 始终有可用的回退（Claude Opus）
- **基于文件的 I/O 模式**：使用带时间戳的 `.tmp/` 目录以避免冲突
- **执行时间**：2-5 分钟（Codex xhigh 推理）、1-3 分钟（Agent CLI/Claude）
- **成本考量**：因级别而异
  - Codex：每次评审约 $0.50-1.50（成本最高，高级特性）
  - Agent CLI：取决于 Cursor Agent CLI 配置（中间）
  - Claude：每次评审约 $1.00-3.00（成本较高，始终可用）
- **安全性**：所有级别都使用只读限制（沙箱/工具）
- **质量收益**：第一、二级支持事实核查和彻底的分析
- **回退保障**：Claude Code 始终可用，作为最终回退
